...
export const addReviewToCourse=async(instId,review)=>{
    const result=await fetch(`http://localhost:8000/trainee/addReviewToInst/${localStorage.getItem("token")}/${instId}/${review}`)
    const j=await result.json();
    if(j=="error"){
...
export const addReviewToInst=async(courseId,review)=>{
    const result=await fetch(`http://localhost:8000/trainee/addReviewToCourse/${localStorage.getItem("token")}/${courseId}/${review}`)
    const j=await result.json();
    if(j=="error"){
